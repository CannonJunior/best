...
function buildInitialPrompt(location: string, dateContext: string): string {
  return `Generate a "best things this week" briefing for ${location}. Today is ${dateContext}.

Use web_search to find:

1. Upcoming events in ${location} in the next 7 days — search:
   - "${location} events this week"
   - "${location} concerts this week"
...
3. Outdoor and seasonal picks:
   - "${location} outdoor events"
   - "${location} hiking trail conditions" (if relevant to season)

4. Arts and sports:
   - "${location} museum exhibit opening"
   - "${location} sports games this week"

Prioritize picks that are happening only this week or are newly opened. Skip anything that runs year-round without a specific weekly hook.
